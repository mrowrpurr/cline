...
export const SYSTEM_PROMPT = async (
	...
	const overridePath = path.join(os.homedir(), ".cline", "system-prompt.js")

	if (fs.existsSync(overridePath)) {
		try {
			const customFn = require(overridePath)
			if (typeof customFn === "function") {
				return await customFn(cwd, supportsBrowserUse, mcpHub, browserSettings)
			}
		} catch (err) {
...
- PLAN MODE: In this special mode, you have access to the plan_mode_respond tool.
 ...
 - You may use tools in Plan Mode to read files, inspect project structure, explore version control history, or gather other non-destructive context. Avoid tools that modify implementation code or execute commands unless the file is clearly for planning purposes (e.g. \`PROJECT_IDEAS.md\`).
